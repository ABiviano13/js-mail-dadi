...
const mailPresenti = ['IntesaSanPaolo', 'BurgerKing', 'Boolean', 'Linkedin', 'Stradivarius', 'Amazon'];
console.log(mailPresenti);
//3. Chiedere all'utente che mail vuole cercare
const mailDaCercare = prompt('Desideri cercare una mail? Quale sarebbe il mittente?');
...
for( let i = 0; i < mailPresenti.length; i++){
    const mailCorrente = mailPresenti[i];
    console.log(mailCorrente);

    // const mailUguali = mailCorrente === mailDaCercare;
    // console.log(mailUguali)

    if(mailCorrente === mailDaCercare){
        esito = true;
    }
}
